...
bot = new Tbot('492365773:AAEKRRYEwvE-_6_6Eg5utPIjW_6tQuAR8AU', {pulling: true});
...
function startBot()
{
	...
	([
		...
		function (callback)
		{
			let collection = db.collection('telegram');			
		...
		function (callback)
		{
			try
			{
				(async() =>
				{
					...
					await browser.close();
					bot.sendPhoto("@healoba", __dirname + '/pic.png', {caption: 'ادامه مقاله رو در سایت بخوانید. لینک مقاله:' + firstArtUrl})
		...
		function (callback)
		{
			...
			pandoc(FAText , '-f html -t commonmark' , function (err, res)
			{
				...
				FAText = res;		
				bot.sendMessage("@healoba", FAText, options);	
