...
.get(function(req , res, next)
{
	...
	([
		function (callback)
		{
			DBMain.node_byPath(path , consV.database.enc.other.CollName , function(err, resul)
			{
				if(err == true)
				{
					return next();
		...
		function (callback)
		{
			if(tree_nodeid == null)
			{
				DBMain.parentId_by_nodeId(consV.database.enc.other.CollName, target_node_id, function (err, re)
				{
					tree_nodeid = re;
					callback(null);						
				});
		...
		function (callback)
		{
			DBMain.loc_byPath(path , consV.database.enc.other.CollName , req.params[0], function(err, loc_path)
			{
				if(err)
				{
					callback(err);
				...
				{
					loc_path.splice(0,1);
					let location = ['دانشنامه' , 'متفرقه'].concat(loc_path);
		...
		function (callback)
		{
			DBArticles.article_approves(target_node_id, consV.database.enc.other.CollName, function(err , ArtApprvs)
			{
				if(err)
				{
					callback(err);
		...
		function (callback)
		{
			DBArticles.article_resources_WUsersAResInfo(target_node_id, consV.database.enc.other.CollName, function(err , res)
			{
				if(err || res == null)
				{
		...
		function (callback)
		{
			DBMain.build_tree(consV.database.enc.other.CollName, tree_nodeid || target_node_id, 4 , function(err, tree , depth)
			{
				if(err)
				{
					callback(err);
		...
		{			
			if(tree_nodeid != null)
			{
				DBMain.url_by_NodeId(tree_nodeid, consV.database.enc.other.CollName, function(err , url)
				{
					if( url == null )
					{
		...
		function (callback)
		{
			DBMain.branch_by_NodeId(target_node_id, consV.database.enc.other.CollName, req.params[0], function(err , br)
			{
				if( br == null )
				{
					err = consV.codes.db.docNotFound;
	...
	function(err , result)
	{
		...
		res.render("encyclopedia/other/home" ,
		{
			title: title,
			location: result[2],
